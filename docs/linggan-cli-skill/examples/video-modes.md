...
**模式与图片数量是绑死的**：给了 `first_frame` 就是图生视频，只能有 1 张图；给了 `first_frame` + `last_frame` 就是首尾帧，只能有 2 张图（多一张就会收到「首尾帧参考只收两张图片（首帧 + 尾帧），不能额外带参考图」）。想在首尾帧之外再带风格/角色参考图，**上游协议不允许**（参考图与首尾帧不能混用），不是命令行少传了参数——这种需求只能改用「全能参考」（多图 + `reference_to_video`），而那条路不表达首尾帧。
