...
func TestCloudAgentToolSchemaStaysCompact(t *testing.T) {
	...
	// 体积预算：上游 20 个工具实测 22,777 字节（预算 24000）；新增 canvas_arrange_nodes
	// 并把描述、patch 字段补齐后实测 21 个工具 24,788 字节，预算上调到 25000；给
	// generate_media 补 videoEditOperation（纯图参考无法推导出多图全能参考）后实测 22 个
	// 工具 25,237 字节，因此上调到 25,500（约 1% 余量）。新增工具或字段时请重新测量并有意识
	// 地调整这个数字，而不是让 schema 悄悄膨胀（它每一步都要发、还在前缀最前面）。
	if len(raw) > 25500 {
		t.Fatalf("平台工具 schema 体积 %d 字节超出预算 25500：请压缩描述或显式调整预算", len(raw))
	}
}
