...
function fixture() {
    const channel = createModelChannel({
        id: "comfy", name: "Comfy", scope: "system", models: ["h3-multi"],
        modelCosts: [{
            model: "h3-multi", displayName: "MiniMax H3", channelLabel: "多图一致性", capability: "video",
            pricePolicy: "channel", billingMode: "per_second", unitPriceMicrocredits: 100_000,
            logicalPriceTiers: [480, 720].map((resolution, index) => ({
                selector: { vquality: resolution + "p", videoSeconds: "6" }, resolution: resolution + "p", videoSeconds: 6,
                billingMode: "per_second", unitPriceMicrocredits: (index + 1) * 100_000,
                inputTokenPriceMicrocredits: 0, outputTokenPriceMicrocredits: 0, cachedTokenPriceMicrocredits: 0,
            })),
        }],
    });
...
test("candidate rows show their configured prices instead of applying the previous model's specifications", () => {
    ...
    for (const theme of [canvasThemes.light, canvasThemes.dark]) {
        const markup = renderToStaticMarkup(<ModelLabel config={config} model={config.model} capability="video" theme={theme}
            creationVariant showConfiguredModelName={false} requirements={requirements} showPrice showDescription />);
...
test("candidate rows do not disguise missing prices as zero", () => {
    ...
    const markup = renderToStaticMarkup(<ModelLabel config={config} model={config.model} capability="video" theme={canvasThemes.dark}
        creationVariant showConfiguredModelName={false} requirements={requirements} showPrice showDescription />);
...
test("the selected model and request estimate still require an exact specification match", () => {
    const config = fixture();
    const render = (value: ModelRequirements) => renderToStaticMarkup(<ModelPicker config={config} value={config.model} capability="video"
        requirements={value} onChange={() => {}} />);
    expect(render(requirements)).toContain("当前规格无报价");
    const cost = (value: ModelRequirements) => requestCreditCost({ channelMode: "remote", modelCosts: config.channels[0].modelCosts,
        model: "h3-multi", capability: "video", config, requirements: value, seconds: "6" });
