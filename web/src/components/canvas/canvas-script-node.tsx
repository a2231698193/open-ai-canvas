...
function generationBatchSummary(batch: CanvasGenerationBatch) {
    ...
    const generating = count("waiting") + count("submitting") + count("queued") + count("running");
    const stopped = count("cancelled");
    return `${generationBatchModeLabel(batch)}${batch.status === "completed" ? "完成" : batch.status === "cancelled" ? "已停止" : "中"} · 完成 ${count("succeeded")}/${batch.items.length} / 失败 ${count("failed")} / 生成中 ${generating}${stopped ? ` / 已停止 ${stopped}` : ""}`;
}
...
function generationBatchItemLabel(item: CanvasGenerationBatchItem) {
    ...
    const labels: Record<CanvasGenerationBatchItemStatus, string> = { waiting: "生成中", submitting: "生成中", queued: "生成中", running: "生成中", succeeded: "成功", failed: "失败", cancelled: "已停止" };
