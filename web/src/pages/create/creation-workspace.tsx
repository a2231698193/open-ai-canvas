...
function MediaResult({ item, onRetryFailure, onCreateVariant, onContinueCanvas, openingCanvas }: { item: CreationMessage; onRetryFailure: () => void; onCreateVariant: () => void; onContinueCanvas: (ids?: string[]) => void; openingCanvas: boolean }) {
    ...
    return <div className="creation-media-result">
        ...
        <div className="creation-media-actions"><span>{isVideo ? "视频结果" : `${displayResultUrls.length} 张图片`}</span><Button type="link" size="small" loading={openingCanvas} disabled={!canContinueWithResults} title={canContinueWithResults ? undefined : "素材保存完成后才能转入画布"} onClick={() => onContinueCanvas(resultAssetIds)}>添加到画布</Button>{displayResultUrls.map((url, index) => { const fileName = isVideo ? "生成视频.mp4" : displayResultUrls.length > 1 ? `生成图片-${index + 1}.png` : "生成图片.png"; return <a key={`${url}-download`} href={mediaDownloadUrl(url, fileName)} download={fileName} onClick={(event) => { event.preventDefault(); void downloadNamedFile(url, fileName).catch((error) => message.error(error instanceof Error ? error.message : "下载失败")); }}>{displayResultUrls.length > 1 ? `下载 ${index + 1}` : <><Download />下载</>}</a>; })}</div>
        <CreationMediaPreviewModal url={previewUrl} type={previewType} onClose={() => setPreviewUrl("")} />
    </div>;
}
