...
// 文本任务的 result_json 外层是 {mode, reasoning, text} 包装，内层才是模型输出的 JSON 字符串。
function storyboardTextPayload(raw: string) {
    try {
...
export function storyboardRowsFromTask(task: GenerationTask) {
    const rawResult = task.resultJson || "{}";
    const parsed = parseStoryboardRowsPayload(rawResult);
    // 模板路径（metadata.promptTemplateOperation=storyboard_plan）在服务端尚未物化 rows 时落的是
    // shots 数组，需要兼容解析；两段解析都取不到镜头行时，由下面的缺行校验统一报错。
    const result = Array.isArray(parsed.rows) && parsed.rows.length
        ? parsed
        : storyboardRowsFromTextResult(storyboardTextPayload(rawResult), task.inputJson) || parsed;
